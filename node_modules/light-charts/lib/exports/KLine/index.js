...
var KLine = function KLine(_ref) {
  ...
  var _useState = (0, _react.useState)(function () {
      var map = new Map([]);
      var colorMap = {
        5: _light.LINE_COLOR.YELLOW,
        10: _light.LINE_COLOR.ORANGE,
        20: _light.LINE_COLOR.BLUE
      };
  ...
  var renderCandles = function renderCandles(dataYAxisPoint, candleWidth) {
    for (var i = 0, candleLength = dataYAxisPoint.length; i < candleLength; i++) {
      renderCandle(dataYAxisPoint[i], (0, _common.xAxisTickPointX)(i, originalPointX, xAxisItemWidth), candleWidth);
    }
  };
  ...
  var oneByOneRenderCandle = function oneByOneRenderCandle(dataYAxisPoint, candleWidth) {
    for (var i = 0, candleLength = dataYAxisPoint.length; i < candleLength; i++) {
      (function (j) {
        setTimeout(function () {
          renderCandle(dataYAxisPoint[j], (0, _common.xAxisTickPointX)(j, originalPointX, xAxisItemWidth), candleWidth);
        }, j * 100);
      })(i);
    }
  };
  ...
  var renderCandle = function renderCandle(dataItem, xAxisItemPointX, candleWidth) {
    ...
    if (closingPrice < openingPrice) {
      // 涨
      candleColor = _light.COLOR.RED;
      secondPointY = closingPrice;
      thirdPointY = openingPrice;
    } else {
      candleColor = _light.COLOR.GREEN;
      secondPointY = openingPrice;
      thirdPointY = closingPrice;
    }

    // 绘制蜡烛上影线
    (0, _common.renderLine)(ctx, xAxisItemPointX, highestPrice, xAxisItemPointX, secondPointY, candleColor, 1);

    // 绘制蜡烛下影线
    (0, _common.renderLine)(ctx, xAxisItemPointX, lowestPrice, xAxisItemPointX, thirdPointY, candleColor, 1);
    ...
    if (candleColor === _light.COLOR.RED) {
      ctx.stroke();
    } else {
      ctx.fill();
    }
  ...
  var renderTipCanvas = function renderTipCanvas() {
    ...
    tipCanvas.addEventListener('mousemove', function (e) {
      ...
      ctxTip.strokeStyle = _light.BORDER_COLOR.SECOND;
      ctxTip.stroke();
      ...
      ctxTip.strokeStyle = _light.BORDER_COLOR.PRIMARY;
      ctxTip.stroke();

      // 绘制y轴tip文字背景框
      ctxTip.beginPath();
      ctxTip.rect(originalPointX, offsetY - xyAxisTipBoxHeight / 2, xyAxisTipBoxWidth, xyAxisTipBoxHeight);
      ctxTip.fillStyle = _light.BORDER_COLOR.SECOND;
      ctxTip.fill();

      // 绘制y轴tip文字
      (0, _common.renderText)(ctxTip, originalPointX + 25, offsetY, (0, _common.yAxisTickText)(yAxisOriginPointY - offsetY, maxPrice, minPrice, yAxisHeight), 'center', _light.COLOR.WHITE);
      ...
      (0, _common.renderText)(ctxTip, offsetX, yAxisOriginPointY + xyAxisTipBoxHeight / 2, dataSource.map(function (x) {
        return x.date;
      })[xTipIndex] || '', 'center', _light.COLOR.WHITE);
      ...
      ctxTip.fillStyle = _light.BG_COLOR.COMPONENT;
      ...
      cloneDataArr.forEach(function (_ref2, i) {
        ...
        (0, _common.renderText)(ctxTip, tipInfoPointX + tipInfoElWidth - padding_horizontal, y, value, 'right', i !== 0 ? _light.TEXT_COLOR.PRIMARY : _light.TEXT_COLOR.RED, '13px');
      });
    }, false);
  };
  ...
  var renderCanvas = function renderCanvas() {
    ...
    var tranPriceToOrdinate = function tranPriceToOrdinate(price) {
      return yAxisOriginPointY - (0, _common.valueHeight)(price, maxPrice, minPrice, yAxisHeight);
    };
    ...
    for (var i = 0; i < yAxisSplitNumber; i++) {
      ...
      (0, _common.renderLine)(ctx, sx, y, xAxisVertexX, y, _light.BORDER_COLOR.SECOND); // 网格线
    }
    ...
    for (var _i = 0; _i < xAxisItemLength; _i++) {
      ...
      if (_i % remainder === 0) {
        (0, _common.renderText)(ctx, xAxisTickX, yAxisOriginPointY + tickLength + 10, dataSource.map(function (x) {
          return x.date;
        })[_i], 'center', _light.TEXT_COLOR.PRIMARY);
        (0, _common.renderLine)(ctx, xAxisTickX, yAxisOriginPointY, xAxisTickX, yAxisOriginPointY + tickLength, _light.BORDER_COLOR.SECOND); // 刻度线
        (0, _common.renderLine)(ctx, xAxisTickX, yAxisOriginPointY, xAxisTickX, yAxisVertexY, _light.BORDER_COLOR.SECOND); // 网格线
      }
    }
    ...
    var getMAControlPointInfo = function getMAControlPointInfo(day) {
      ...
      for (var _i3 = 0; _i3 < xAxisItemLength; _i3++) {
        var pricePointX = (0, _common.xAxisTickPointX)(_i3, originalPointX, xAxisItemWidth);
...
var _default = _react["default"].memo(KLine);
